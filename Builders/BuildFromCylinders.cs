...
//         BispatialDictionary<VIdx, Vert> NewVerts = [];
//         BispatialDictionary<EIdx, Edge> NewEdges = new();
...
//                 NewVerts[v_idx] = VertUtil.ToVertWithSortedEdgesAndPolys(old_vert, v_idx, NewEdges.Forwards, NewPolys);
//             }

//             Surface ret = new(NewVerts.Forwards, NewEdges.Forwards, NewPolys);
...
//                 Vector3[] outer_poly_verts = [..outer_poly.Verts.Select(x => NewVerts[x].Position)];
...
//             foreach(Vert vert in poly.Verts.Select(x => NewVerts[x]))
//             {
//                 vert.RemovePoly(p_idx);
//             }

//             foreach(Edge edge in poly.Edges.Select(x => NewEdges[x]))
//             {
...
//         {
//             return PolyUtil.PolyCentre([.. poly.Verts.Select(x => NewVerts[x].Position)]);
//         }

//         private Vector3 PolyNormal(Poly poly)
//         {
//             return PolyUtil.PolyNormal([..poly.Verts.Select(x => NewVerts[x].Position)]);
//         }
...
//                 Edge edge = new(v_idx, next_v_idx, GetEdgeBounds(v_idx, next_v_idx));
...
//             Vert[] verts = poly.Verts.Select(x => NewVerts[x]).ToArray();
...
//             => new (outer_poly.Verts, Topology.Outside);

//         private VertLoop MakeInnerPolyLoop(Poly inner_poly)
//             => new (inner_poly.Verts, Topology.Inside);
